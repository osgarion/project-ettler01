---
title: "Adverse effects of bexarotene therapy in cutaneous T-cell lymphoma"
author: "*Jiri Baloun*"
...
d04 <- import(here::here("data","processed","Bexaroten 20250419_processed.xlsx")) |>
  ...
  dplyr::filter(!.data$initials %in% c("CZ", "HZ")) |> # !!!!!!!!!!
  mutate(
...
df <- d04 |>
  ...
  dplyr::mutate(
    ...
    overweight = dplyr::if_else(!is.na(.data$bmi) & .data$bmi >= 25, 1, 0),
    obese = dplyr::if_else(!is.na(.data$bmi) & .data$bmi >= 30, 1, 0)
  )
...
| Reason for discontinuation | 0 = treatment continues; >0 = treatment discontinued (coded reason) | discontinuation_reason |
| Discontinued because of AE? | 1=yes, 0=no | discontinued_due_to_ae |
| TTNT | Time to next treatment (months; censored if no next treatment) | ttnt |
| TTNT achieved? | 1 = yes (next treatment given), 0 = no (censored; no next treatment) | ttnt_achieved |
| Comorbidities |  | comorbidities |
| Dyslipidemia before bexarotene? | 1=yes, 0=no | dyslipidemia_before |
...
covars <- c("age","sex","bmi","overweight","obese","ps_ecog","first_syst_th","monotherapy","ae_grade_3_4","final_dose_mg_m2")
...
##### overweight  {.unnumbered}  

```{r adjusted_plots_overweight, message=FALSE, warning=FALSE}
p_overweight_adj <- try(plot_stacked_by_stage(df, covar = resolve_or_identity(df, "overweight")), silent = TRUE)
if (!inherits(p_overweight_adj, "try-error") && !is.null(p_overweight_adj)) print(p_overweight_adj) else cat("No data for overweight.\n")
```

##### obese  {.unnumbered}  

```{r adjusted_plots_obese, message=FALSE, warning=FALSE}
p_obese_adj <- try(plot_stacked_by_stage(df, covar = resolve_or_identity(df, "obese")), silent = TRUE)
if (!inherits(p_obese_adj, "try-error") && !is.null(p_obese_adj)) print(p_obese_adj) else cat("No data for obese.\n")
```
...
covars_lin <- c("age","sex","bmi","overweight","obese","ps_ecog","first_syst_th","monotherapy","ae_grade_3_4","final_dose_mg_m2")
...
#### Sensitivity: BMI < 40 vs all (ANOVA) {.unnumbered}

```{r adjusted_linear_bmi_sensitivity, message=FALSE, warning=FALSE, results="asis"}
# Sensitivity: model on BMI < 40 only, formatted like linear results + plot
col_y     <- resolve_or_identity(df, "response_time_to_2")
col_stage <- resolve_or_identity(df, "stage_early")
col_bmi   <- resolve_or_identity(df, "bmi")

d_trim <- df |>
  dplyr::select(y = dplyr::all_of(col_y), stage = dplyr::all_of(col_stage), bmi = dplyr::all_of(col_bmi)) |>
  dplyr::filter(!is.na(y), is.finite(y), y > 0, !is.na(stage), !is.na(bmi), bmi < 40) |>
  dplyr::mutate(stage = as.factor(stage))

fmt_p <- function(p) ifelse(is.na(p), NA_character_, ifelse(p < 0.001, "<0.001", sprintf("%.3f", p)))

if (nrow(d_trim) >= 5 && length(unique(d_trim$stage)) > 1) {
  fit_trim <- try(stats::lm(y ~ stage + bmi, data = d_trim), silent = TRUE)
  if (!inherits(fit_trim, "try-error")) {
    tt <- broom::tidy(fit_trim, conf.int = TRUE)
    st <- tt |> dplyr::filter(stringr::str_detect(.data$term, "^stage")) |> dplyr::slice(1)
    bm <- tt |> dplyr::filter(.data$term == "bmi") |> dplyr::slice(1)
    if (nrow(st) == 1 && nrow(bm) == 1) {
      tab <- tibble::tibble(
        Outcomes = col_y,
        Covariate = "bmi (<40)",
        `estimate (stage_early)` = sprintf("%.3f", st$estimate),
        `p-value (stage_early)`  = fmt_p(st$p.value),
        `95%CI (stage_early)`    = sprintf("[%.2f, %.2f]", st$conf.low, st$conf.high),
        `estimate (covariate)`   = sprintf("%.3f", bm$estimate),
        `p-value (covariate)`    = fmt_p(bm$p.value),
        `95%CI (covariate)`      = sprintf("[%.2f, %.2f]", bm$conf.low, bm$conf.high)
      )
      print(
        knitr::kable(tab, align = "l",
                     col.names = c("Outcomes","Covariate","estimate (stage_early)","p-value (stage_early)","95%CI (stage_early)",
                                   "estimate (covariate)","p-value (covariate)","95%CI (covariate)")) |>
          kableExtra::kable_styling(full_width = FALSE, bootstrap_options = c("striped","hover","condensed","responsive"))
      )
    } else {
      cat("Model terms not estimated for stage_early or bmi.\n")
    }
  } else {
    cat("Model fitting failed for BMI < 40 subset.\n")
  }

  # Plot in same style as Plots - linear
  pal <- scales::hue_pal()(length(levels(d_trim$stage))); names(pal) <- levels(d_trim$stage)
  p <- ggplot2::ggplot(d_trim, ggplot2::aes(x = bmi, y = y, color = stage)) +
    ggplot2::geom_point(alpha = 0.6, size = 1.7) +
    ggplot2::geom_smooth(method = "lm", se = FALSE) +
    ggplot2::scale_color_manual(values = pal, name = "stage_early") +
    ggplot2::labs(x = "bmi", y = col_y, title = "Response time to ~ BMI (<40) by stage") +
    ggplot2::theme_minimal(base_size = 12)
  print(p)
} else {
  cat("Insufficient data for BMI < 40 model (need >=5 rows and both stage levels).\n")
}
```
...
covars_surv <- c('age','sex','bmi','overweight','obese','ps_ecog','first_syst_th','monotherapy','ae_grade_3_4','final_dose_mg_m2')
...
This retrospective, hypothesis-generating analysis evaluated whether **early stage disease (stage_early)** and selected clinical covariates were associated with key bexarotene outcomes in cutaneous T-cell lymphoma.

- **Response achieved (binary):** In the unadjusted model, stage_early was not clearly associated with response (OR 1.79, 95% CI 0.33–13.65; p = 0.521), although estimated response probability was numerically higher in stage_early=1 (0.71 vs 0.58).
- **Time-to-event outcomes:** For **time to next treatment (TTNT)**, stage_early=1 showed a lower hazard (HR 0.26, 95% CI 0.08–0.80; p = 0.018), consistent with a longer TTNT compared with stage_early=0 (KM median 44 vs 6 months). For **treatment duration**, **response duration**, and **disease-control duration**, estimates were imprecise and not statistically conclusive in this dataset.
- **Time to response (continuous, y > 0):** Stage_early was not clearly associated with time to response in the main model (mean difference −1.24 months, 95% CI −4.18 to 1.69; p = 0.392).

Overall, the clearest signal in the current dataset is a potentially longer TTNT among patients classified as early stage, while other endpoints did not show robust stage-related differences. Results should be interpreted cautiously due to small sample sizes, complete-case handling, and the exploratory nature of the analyses.
