...
# Duplicate response_time_to with zeros set to NA
if ("response_time_to" %in% names(d04)) {
  d04 <- d04 |> dplyr::mutate(response_time_to_2 = dplyr::na_if(.data$response_time_to, 0))
}
...
# Build narrative for response_time_to ~ stage (y > 0)
...
# Jitter plot of response_time_to by stage with bootstrap median and 95% CI (5000x)
col_stage <- resolve_col(df, "stage_early", alias_map$stage_early)
col_response_time_to <- resolve_col(df, "response_time_to", alias_map$response_time_to)
d_lin <- df |>
  dplyr::select(y = dplyr::all_of(col_response_time_to), stage = dplyr::all_of(col_stage)) |>
...
if (nrow(d_lin) >= 2) {
  ...
  p <- ggplot2::ggplot(d_lin, ggplot2::aes(x = stage, y = y, color = stage)) +
    ...
    ggplot2::labs(x = "stage_early", y = "response_time_to", title = "Response time to by stage (median with 95% CI)") +
...
covars <- c("age","sex","bmi","ps_ecog","first_syst_th","monotherapy","ae_grade_3_4")
...
# Stage-specific covariate effects (within-stage) via emmeans
if (!requireNamespace("emmeans", quietly = TRUE)) {
  cat("emmeans package not available; stage-wise covariate effects skipped.\n")
} else {
  fmt_p_str <- function(p) ifelse(is.na(p), NA_character_, ifelse(p < 0.001, "<0.001", sprintf("%.3f", p)))
  fmt_ci_str <- function(l, u) ifelse(is.na(l) | is.na(u), NA_character_, sprintf("[%.2f, %.2f]", l, u))

  compute_stagewise <- function(df, outcome_col, stage_col, covar_col) {
    safe_num <- function(x) {
    ...
    d_mod <- df |>
      dplyr::select(y = dplyr::all_of(outcome_col), stage = dplyr::all_of(stage_col), x = dplyr::all_of(covar_col)) |>
      dplyr::mutate(y = as.factor(y), stage = as.factor(stage)) |>
      dplyr::filter(!is.na(y), !is.na(stage), !is.na(x))
    if (nrow(d_mod) == 0 || length(unique(d_mod$stage)) < 2) return(NULL)
    # Special handling for numeric age/bmi: report subgroup means instead of OR
    is_special_mean <- covar_col %in% c("age", "bmi")
    if (is_special_mean && is.numeric(d_mod$x)) {
      # compute bootstrap 95% CI for mean within each stage
      boot_mean_ci <- function(v, n = 2000L, conf = c(0.025, 0.975)) {
        v <- v[is.finite(v)]
        if (length(v) < 2) return(c(mean = mean(v), l = NA_real_, u = NA_real_))
        b <- replicate(n, mean(sample(v, replace = TRUE)))
        c(mean = mean(v), l = stats::quantile(b, conf[1], names = FALSE), u = stats::quantile(b, conf[2], names = FALSE))
      }
      st_levels <- levels(d_mod$stage)
      # Resolve stage labels robustly
      st_vals <- st_levels
      stage0_lab <- if (length(st_vals) >= 1) (if ("0" %in% st_vals) "0" else st_vals[1]) else NA_character_
      stage1_lab <- if (length(st_vals) >= 2) (if ("1" %in% st_vals) "1" else st_vals[2]) else NA_character_

      out <- tibble::tibble(
        `estimate (stage_early = 0)` = NA_real_,
        `p-value (stage_early = 0)` = NA_character_,
        `95%CI (stage_early = 0)` = NA_character_,
        `estimate (stage_early = 1)` = NA_real_,
        `p-value (stage_early = 1)` = NA_character_,
        `95%CI (stage_early = 1)` = NA_character_
      )
      if (!is.na(stage0_lab)) {
        v0 <- d_mod$x[as.character(d_mod$stage) == stage0_lab]
        ci0 <- boot_mean_ci(v0)
        out$`estimate (stage_early = 0)` <- ci0[["mean"]]
        out$`95%CI (stage_early = 0)` <- fmt_ci_str(ci0[["l"]], ci0[["u"]])
        # p-value: association within stage via logistic regression slope
        d0 <- d_mod[as.character(d_mod$stage) == stage0_lab, , drop = FALSE]
        p0 <- tryCatch({
          m0 <- stats::glm(y ~ x, data = d0, family = stats::binomial())
          as.numeric(broom::tidy(m0) |>
            dplyr::filter(.data$term == "x") |>
            dplyr::slice(1) |>
            dplyr::pull(.data$p.value))
        }, error = function(e) NA_real_)
        out$`p-value (stage_early = 0)` <- fmt_p_str(p0)
      }
      if (!is.na(stage1_lab)) {
        v1 <- d_mod$x[as.character(d_mod$stage) == stage1_lab]
        ci1 <- boot_mean_ci(v1)
        out$`estimate (stage_early = 1)` <- ci1[["mean"]]
        out$`95%CI (stage_early = 1)` <- fmt_ci_str(ci1[["l"]], ci1[["u"]])
        d1 <- d_mod[as.character(d_mod$stage) == stage1_lab, , drop = FALSE]
        p1 <- tryCatch({
          m1 <- stats::glm(y ~ x, data = d1, family = stats::binomial())
          as.numeric(broom::tidy(m1) |>
            dplyr::filter(.data$term == "x") |>
            dplyr::slice(1) |>
            dplyr::pull(.data$p.value))
        }, error = function(e) NA_real_)
        out$`p-value (stage_early = 1)` <- fmt_p_str(p1)
      }
      return(out)
    }

    # Default: fit interaction model and use emmeans/emtrends to report OR
    # Treat numeric binary (0/1) predictors as factors to obtain a clear 1 vs 0 OR
    if (is.numeric(d_mod$x)) {
      ux <- unique(d_mod$x[!is.na(d_mod$x)])
      if (length(ux) > 0 && all(ux %in% c(0,1))) {
        d_mod$x <- factor(as.integer(d_mod$x))
      }
    }
    d_mod <- d_mod |> dplyr::mutate(x = coerce_predictor(x))
    m_int <- tryCatch(stats::glm(y ~ stage * x, data = d_mod, family = stats::binomial()), error = function(e) NULL)
    if (is.null(m_int)) return(NULL)

    # Note: do NOT include 'outcomes' or 'covariates' here to avoid name collision on unnest
    out <- tibble::tibble(
      `estimate (stage_early = 0)` = NA_real_,
      `p-value (stage_early = 0)` = NA_character_,
      `95%CI (stage_early = 0)` = NA_character_,
    ...
    if (is.factor(d_mod$x)) {
      em <- try(emmeans::emmeans(m_int, specs = ~ x | stage), silent = TRUE)
      if (!inherits(em, "try-error")) {
        cn <- try(emmeans::contrast(em, method = "trt.vs.ctrl", ref = 1), silent = TRUE)
        sm <- try(as.data.frame(summary(cn, infer = TRUE)), silent = TRUE)
        if (!inherits(sm, "try-error") && nrow(sm) > 0) {
          # Determine CI column names robustly
          lo_name <- if ("lower.CL" %in% names(sm)) "lower.CL" else if ("asymp.LCL" %in% names(sm)) "asymp.LCL" else NA_character_
          up_name <- if ("upper.CL" %in% names(sm)) "upper.CL" else if ("asymp.UCL" %in% names(sm)) "asymp.UCL" else NA_character_
          # Resolve stage labels present in summary
          st_vals <- if ("stage" %in% names(sm)) unique(as.character(sm$stage)) else levels(d_mod$stage)
          stage0_lab <- if (length(st_vals) >= 1) (if ("0" %in% st_vals) "0" else st_vals[1]) else NA_character_
          stage1_lab <- if (length(st_vals) >= 2) (if ("1" %in% st_vals) "1" else st_vals[2]) else NA_character_
          # Pick one contrast row per stage label
          if (!is.na(stage0_lab)) {
            r0 <- sm[as.character(sm$stage) == stage0_lab, , drop = FALSE]
            if (nrow(r0) >= 1) {
              est0 <- safe_num(r0$estimate)
              lo0  <- if (!is.na(lo_name)) safe_num(r0[[lo_name]]) else NA_real_
              up0  <- if (!is.na(up_name)) safe_num(r0[[up_name]]) else NA_real_
              p0v  <- safe_num(r0$p.value)
              out$`estimate (stage_early = 0)` <- ifelse(is.na(est0), NA_real_, exp(est0))
              out$`p-value (stage_early = 0)` <- fmt_p_str(p0v)
              out$`95%CI (stage_early = 0)` <- fmt_ci_str(ifelse(is.na(lo0), NA_real_, exp(lo0)), ifelse(is.na(up0), NA_real_, exp(up0)))
            }
          }
          if (!is.na(stage1_lab)) {
            r1 <- sm[as.character(sm$stage) == stage1_lab, , drop = FALSE]
            if (nrow(r1) >= 1) {
              est1 <- safe_num(r1$estimate)
              lo1  <- if (!is.na(lo_name)) safe_num(r1[[lo_name]]) else NA_real_
              up1  <- if (!is.na(up_name)) safe_num(r1[[up_name]]) else NA_real_
              p1v  <- safe_num(r1$p.value)
              out$`estimate (stage_early = 1)` <- ifelse(is.na(est1), NA_real_, exp(est1))
              out$`p-value (stage_early = 1)` <- fmt_p_str(p1v)
              out$`95%CI (stage_early = 1)` <- fmt_ci_str(ifelse(is.na(lo1), NA_real_, exp(lo1)), ifelse(is.na(up1), NA_real_, exp(up1)))
            }
          }
        }
      }
    } else {
      tr <- try(emmeans::emtrends(m_int, specs = ~ stage, var = "x"), silent = TRUE)
      if (!inherits(tr, "try-error")) {
        smt <- try(as.data.frame(summary(tr, infer = TRUE)), silent = TRUE)
        if (!inherits(smt, "try-error") && nrow(smt) > 0) {
          lo_name <- if ("lower.CL" %in% names(smt)) "lower.CL" else if ("asymp.LCL" %in% names(smt)) "asymp.LCL" else NA_character_
          up_name <- if ("upper.CL" %in% names(smt)) "upper.CL" else if ("asymp.UCL" %in% names(smt)) "asymp.UCL" else NA_character_
          trend_name <- if ("emtrends" %in% names(smt)) "emtrends" else if ("x.trend" %in% names(smt)) "x.trend" else if ("trend" %in% names(smt)) "trend" else NA_character_
          st_vals <- if ("stage" %in% names(smt)) unique(as.character(smt$stage)) else levels(d_mod$stage)
          stage0_lab <- if (length(st_vals) >= 1) (if ("0" %in% st_vals) "0" else st_vals[1]) else NA_character_
          stage1_lab <- if (length(st_vals) >= 2) (if ("1" %in% st_vals) "1" else st_vals[2]) else NA_character_
          if (!is.na(stage0_lab)) {
            r0 <- smt[as.character(smt$stage) == stage0_lab, , drop = FALSE]
            if (nrow(r0) >= 1) {
              est0 <- if (!is.na(trend_name)) safe_num(r0[[trend_name]]) else NA_real_
              lo0  <- if (!is.na(lo_name)) safe_num(r0[[lo_name]]) else NA_real_
              up0  <- if (!is.na(up_name)) safe_num(r0[[up_name]]) else NA_real_
              p0v  <- safe_num(r0$p.value)
              out$`estimate (stage_early = 0)` <- ifelse(is.na(est0), NA_real_, exp(est0))
          ...
          if (!is.na(stage1_lab)) {
            r1 <- smt[as.character(smt$stage) == stage1_lab, , drop = FALSE]
            if (nrow(r1) >= 1) {
              est1 <- if (!is.na(trend_name)) safe_num(r1[[trend_name]]) else NA_real_
              lo1  <- if (!is.na(lo_name)) safe_num(r1[[lo_name]]) else NA_real_
              up1  <- if (!is.na(up_name)) safe_num(r1[[up_name]]) else NA_real_
              p1v  <- safe_num(r1$p.value)
              out$`estimate (stage_early = 1)` <- ifelse(is.na(est1), NA_real_, exp(est1))
  ...
  stagewise_tbl <- tidyr::expand_grid(outcomes = col_y, covariates = covars) |>
    dplyr::mutate(results = purrr::map(covariates, ~ compute_stagewise(df, outcome_col = col_y, stage_col = col_stage, covar_col = .x))) |>
    dplyr::filter(!purrr::map_lgl(results, is.null)) |>
    tidyr::unnest(results)

  if (nrow(stagewise_tbl) > 0) {
    # Round estimates to 3 decimals for display
    stagewise_disp <- stagewise_tbl |>
      dplyr::mutate(
        `estimate (stage_early = 0)` = ifelse(is.na(`estimate (stage_early = 0)`), NA_character_, sprintf("%.3f", as.numeric(`estimate (stage_early = 0)`))),
        `estimate (stage_early = 1)` = ifelse(is.na(`estimate (stage_early = 1)`), NA_character_, sprintf("%.3f", as.numeric(`estimate (stage_early = 1)`)))
      )
    knitr::kable(stagewise_disp, align = "l",
                 col.names = c(
                   "outcomes","covariates",
                   "estimate (stage_early = 0)","p-value (stage_early = 0)","95%CI (stage_early = 0)",
                   "estimate (stage_early = 1)","p-value (stage_early = 1)","95%CI (stage_early = 1)"
                 )) %>%
      kableExtra::kable_styling(full_width = FALSE, bootstrap_options = c("striped","hover","condensed","responsive")) %>%
      kableExtra::footnote(
        general = "This table evaluates the association of each covariate with the outcome within subgroups defined by stage_early (0 and 1) using a model with the stage × covariate interaction; estimates are subgroup-specific.",
        threeparttable = TRUE
      )
  } else {
    cat("No stage-wise covariate effects available.\n")
  }
}
```
...
plot_scatter_logit_adj <- function(df, covar) {
  ...
  # Match Main models → Plots - main → Linear palette and theme
...
# Adjusted linear models: response_time_to ~ stage_early + covariate (y > 0)
col_stage <- resolve_or_identity(df, "stage_early")
col_lin   <- resolve_or_identity(df, "response_time_to")
covars_lin <- c("age","sex","bmi","ps_ecog","first_syst_th","monotherapy","ae_grade_3_4")
...
# Stage-specific covariate effects (linear): factor x via emmeans(~ x | stage) contrasts; numeric x via emtrends(~ stage, var = "x")
if (!requireNamespace("emmeans", quietly = TRUE)) {
  cat("emmeans package not available; stage-wise linear effects skipped.\n")
} else {
  fmt_p_str <- function(p) ifelse(is.na(p), NA_character_, ifelse(p < 0.001, "<0.001", sprintf("%.3f", p)))
  fmt_ci_str <- function(l, u) ifelse(is.na(l) | is.na(u), NA_character_, sprintf("[%.2f, %.2f]", l, u))

  compute_stagewise_lin <- function(df, outcome_col, stage_col, covar_col) {
    d <- df |>
      dplyr::select(y = dplyr::all_of(outcome_col), stage = dplyr::all_of(stage_col), x = dplyr::all_of(covar_col)) |>
      dplyr::mutate(stage = as.factor(stage)) |>
      dplyr::filter(!is.na(y), !is.na(stage), is.finite(y), y > 0)
    if (nrow(d) == 0 || length(unique(d$stage)) < 2) return(NULL)
    if (is.numeric(d$x)) {
      ux <- unique(d$x[!is.na(d$x)]); if (length(ux) > 0 && all(ux %in% c(0,1))) d$x <- factor(as.integer(d$x))
    }
    d <- d |> dplyr::mutate(x = coerce_predictor(x))
    m_int <- tryCatch(stats::lm(y ~ stage * x, data = d), error = function(e) NULL)
    if (is.null(m_int)) return(NULL)
    out <- tibble::tibble(
      `estimate (stage_early = 0)` = NA_real_,
      `p-value (stage_early = 0)` = NA_character_,
    ...
    if (is.factor(d$x)) {
      ...
      if (!inherits(em, "try-error")) {
        ...
        sm <- try(as.data.frame(summary(cn, infer = TRUE)), silent = TRUE)
        if (!inherits(sm, "try-error") && nrow(sm) > 0) {
          st_vals <- if ("stage" %in% names(sm)) unique(as.character(sm$stage)) else levels(d$stage)
          s0 <- if (length(st_vals) >= 1) (if ("0" %in% st_vals) "0" else st_vals[1]) else NA_character_
          s1 <- if (length(st_vals) >= 2) (if ("1" %in% st_vals) "1" else st_vals[2]) else NA_character_
          if (!is.na(s0)) {
            r0 <- sm[as.character(sm$stage) == s0, , drop = FALSE]
            if (nrow(r0) >= 1) {
              out$`estimate (stage_early = 0)` <- as.numeric(r0$estimate[1])
              out$`p-value (stage_early = 0)` <- fmt_p_str(as.numeric(r0$p.value[1]))
              out$`95%CI (stage_early = 0)` <- fmt_ci_str(as.numeric(r0$lower.CL[1]), as.numeric(r0$upper.CL[1]))
            }
          }
          if (!is.na(s1)) {
            r1 <- sm[as.character(sm$stage) == s1, , drop = FALSE]
            if (nrow(r1) >= 1) {
              out$`estimate (stage_early = 1)` <- as.numeric(r1$estimate[1])
              out$`p-value (stage_early = 1)` <- fmt_p_str(as.numeric(r1$p.value[1]))
              out$`95%CI (stage_early = 1)` <- fmt_ci_str(as.numeric(r1$lower.CL[1]), as.numeric(r1$upper.CL[1]))
            }
          }
        }
      }
      ...
      if (!inherits(tr, "try-error")) {
        ...
        if (!inherits(smt, "try-error") && nrow(smt) > 0) {
          trend_name <- if ("emtrends" %in% names(smt)) "emtrends" else if ("x.trend" %in% names(smt)) "x.trend" else if ("trend" %in% names(smt)) "trend" else NA_character_
          st_vals <- if ("stage" %in% names(smt)) unique(as.character(smt$stage)) else levels(d$stage)
          s0 <- if (length(st_vals) >= 1) (if ("0" %in% st_vals) "0" else st_vals[1]) else NA_character_
          s1 <- if (length(st_vals) >= 2) (if ("1" %in% st_vals) "1" else st_vals[2]) else NA_character_
          if (!is.na(s0)) {
            r0 <- smt[as.character(smt$stage) == s0, , drop = FALSE]
            if (nrow(r0) >= 1) {
              out$`estimate (stage_early = 0)` <- as.numeric(r0[[trend_name]][1])
              out$`p-value (stage_early = 0)` <- fmt_p_str(as.numeric(r0$p.value[1]))
              out$`95%CI (stage_early = 0)` <- fmt_ci_str(as.numeric(r0$lower.CL[1]), as.numeric(r0$upper.CL[1]))
            }
          }
          if (!is.na(s1)) {
            r1 <- smt[as.character(smt$stage) == s1, , drop = FALSE]
            if (nrow(r1) >= 1) {
              out$`estimate (stage_early = 1)` <- as.numeric(r1[[trend_name]][1])
              out$`p-value (stage_early = 1)` <- fmt_p_str(as.numeric(r1$p.value[1]))
              out$`95%CI (stage_early = 1)` <- fmt_ci_str(as.numeric(r1$lower.CL[1]), as.numeric(r1$upper.CL[1]))
            }
          }
  ...
  stagewise_lin <- tidyr::expand_grid(outcomes = col_lin, covariates = covars_lin) |>
    dplyr::mutate(results = purrr::map(covariates, ~ compute_stagewise_lin(df, outcome_col = col_lin, stage_col = col_stage, covar_col = .x))) |>
    dplyr::filter(!purrr::map_lgl(results, is.null)) |>
    tidyr::unnest(results)

  if (nrow(stagewise_lin) > 0) {
    stagewise_lin_disp <- stagewise_lin |>
      dplyr::mutate(
        `estimate (stage_early = 0)` = ifelse(is.na(`estimate (stage_early = 0)`), NA_character_, sprintf("%.3f", as.numeric(`estimate (stage_early = 0)`))),
        `estimate (stage_early = 1)` = ifelse(is.na(`estimate (stage_early = 1)`), NA_character_, sprintf("%.3f", as.numeric(`estimate (stage_early = 1)`)))
      )
    knitr::kable(stagewise_lin_disp, align = "l",
                 col.names = c(
                   "outcomes","covariates",
                   "estimate (stage_early = 0)","p-value (stage_early = 0)","95%CI (stage_early = 0)",
                   "estimate (stage_early = 1)","p-value (stage_early = 1)","95%CI (stage_early = 1)"
    ...
    cat("No stage-wise linear effects available.\n")
  }
}
```
...
col_stage <- resolve_or_identity(df, 'stage_early')
col_y     <- resolve_or_identity(df, 'response_time_to')

lin_end_scatter_by_stage <- function(df, covar) {
  # Resolve columns locally
  y_col     <- resolve_or_identity(df, 'response_time_to')
...
lin_end_jitter_boot_by_stage <- function(df, covar) {
  if (!all(c('response_time_to','stage_early', covar) %in% names(df))) return(NULL)
  
  d <- df |>
    dplyr::transmute(
      y = .data[["response_time_to"]],
  ...
  ggplot2::ggplot(d, ggplot2::aes(x = x, y = y, color = stage)) +
    ...
    ggplot2::labs(
      x = covar, y = "response_time_to",
      title = sprintf("%s by %s (median with 95%% CI)", "response_time_to", covar)
    ) +
    ggplot2::theme_minimal(base_size = 12)
}
...
Where: h(t) is the hazard at time t; h0(t) is the baseline hazard; beta_1 is the adjusted log‑hazard ratio for stage_early (1 vs 0) holding the covariate fixed; beta_2 is the covariate effect (for factors: contrast vs the reference level; for numeric covariates: effect per one‑unit increase). Effects are commonly reported as HR = exp(beta).
...
covars_surv <- c('age','sex','bmi','ps_ecog','first_syst_th','monotherapy','ae_grade_3_4')
...
covars_bmi <- c("age","sex","ps_ecog","first_syst_th","monotherapy","ae_grade_3_4","stage_early")
