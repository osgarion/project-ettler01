...
resolve_proj_path <- function(path) {
  ...
  # Return original path if not found; try_source_first will handle existence
  path
}
...
tryCatch({
  saveRDS(d04, file = "data/d04_lightweight_copy.rds")
  message(" - Saved data/d04_lightweight_copy.rds")
...
resolve_col <- function(df, target, aliases) {
  # Return the first existing alias or the target if present (exact match only)
...
# Prefer non-reference level label for factor contrasts
...
df <- d04 |>
  janitor::clean_names() |> 
  dplyr::mutate(discontinuation_reason = dplyr::if_else(.data$discontinuation_reason == 0, 0, 1),
...
# Ensure discontinuation_reason is binary (0/1) even if various types
...
if (length(num_cols) >= 2 && requireNamespace("corrplot", quietly = TRUE)) {
  ...
  corrplot::corrplot(r, method = "color", type = "lower", tl.col = "black", tl.srt = 45,
                     p.mat = p, sig.level = 0.05, insig = "blank", col = pal(200), addCoef.col = "black", number.cex = 0.5)
  grDevices::dev.off()
  # GGally pairs plot for numeric variables (may be large; cap to 12 vars)
  if (requireNamespace("GGally", quietly = TRUE)) {
    num_df <- df |> dplyr::select(dplyr::all_of(num_cols))
    # keep finite-only columns
    num_df <- num_df |> dplyr::select(where(~ sum(is.finite(.x)) >= 3))
    # Convert pure 0/1 (with NA) variables to factor for better ggpairs handling
    num_df <- num_df |>
      dplyr::mutate(dplyr::across(
        everything(),
        ~ {
          vals <- unique(stats::na.omit(.x))
          if (is.numeric(.x) && length(vals) > 0 && all(vals %in% c(0, 1))) as.factor(.x) else .x
        }
      ))
    if (ncol(num_df) >= 2) {
      max_cols <- 12
      if (ncol(num_df) > max_cols) num_df <- num_df[, seq_len(max_cols), drop = FALSE]
      p_gp <- GGally::ggpairs(num_df, progress = FALSE)
      ggplot2::ggsave("output/figures/eda_ggpairs_numeric_v2.tiff", p_gp, width = 12, height = 10, dpi = 300, device = "tiff", compression = "lzw")
    }
  }
}
...
# DataExplorer HTML EDA report (alongside skimr export)
if (requireNamespace("DataExplorer", quietly = TRUE)) {
  # Writes HTML report into the same EDA tables folder (no CSVs)
  try(
    DataExplorer::create_report(
      df,
      output_file = "dataexplorer_report_v2.html",
      output_dir = "output/tables/eda",
      report_title = "DataExplorer EDA Report (v2)"
    ),
    silent = TRUE
  )
}
...
main_rows <- list(
  tibble::tibble(
    ...
    p_stage0 = log_s0$p.value, effect_stage0 = log_s0$estimate, effect_stage0_ci = fmt_ci(log_s0$conf.low, log_s0$conf.high),
    p_stage1 = log_s1$p.value, effect_stage1 = log_s1$estimate, effect_stage1_ci = fmt_ci(log_s1$conf.low, log_s1$conf.high)
  ),
  dplyr::bind_rows(cox_main) |>
    dplyr::transmute(outcome = .data$outcome, p_all = .data$p.value, effect_all = .data$estimate,
  ...
  tibble::tibble(
    ...
    p_stage0 = lin_s0$p.value, effect_stage0 = lin_s0$estimate, effect_stage0_ci = fmt_ci(lin_s0$conf.low, lin_s0$conf.high),
    p_stage1 = lin_s1$p.value, effect_stage1 = lin_s1$estimate, effect_stage1_ci = fmt_ci(lin_s1$conf.low, lin_s1$conf.high)
  )
)
...
## Statistics — Adjusted Models ----

message("Standalone: Fitting adjusted models ...")
# Fixed covariate set consistent with autogen; keep those present only
...
if (length(covars) == 0) warning("No additional covariates found; producing empty adjusted table.")
adjusted_rows <- list()
for (cv in covars) {
  ...
  if (length(st_levels) >= 1) {
    s0 <- sp_log_emm |> dplyr::filter(as.character(.data$stage) == st_levels[1]) |> dplyr::slice_head(n = 1)
  } else {
    s0 <- tibble::tibble(p.value = NA_real_, estimate = NA_real_, conf.low = NA_real_, conf.high = NA_real_)
  }
...
save_main_plot <- function(row) {
  ...
  if (row$outcome %in% km_labels) {
    ...
    # Overall fit and plot with colored median line
    f_all <- survival::survfit(survival::Surv(time, event) ~ 1, data = d)
    pal_all <- "#1f78b4"
    p_left <- survminer::ggsurvplot(
      f_all, data = d, conf.int = TRUE, risk.table = FALSE,
      ggtheme = theme_minimal() + theme(plot.title = element_text(hjust = 0.5, face = "bold")),
      title = paste0("KM (All): ", sp$label),
      palette = pal_all
    )$plot
    md_all <- try(survminer::surv_median(f_all), silent = TRUE)
    if (!inherits(md_all, "try-error") && length(md_all$median) >= 1 && is.finite(md_all$median[1])) {
      p_left <- p_left + ggplot2::geom_vline(xintercept = md_all$median[1], linetype = "dashed", color = pal_all)
    }

    # By-stage fit and plot; add colored median lines by strata
    f_by <- survival::survfit(survival::Surv(time, event) ~ stage, data = d)
    p_right_list <- survminer::ggsurvplot(
      f_by, data = d, conf.int = TRUE, risk.table = FALSE,
      ggtheme = theme_minimal() + theme(plot.title = element_text(hjust = 0.5, face = "bold")),
      legend.title = "stage_early",
      title = paste0("KM by stage_early: ", sp$label)
    )
    p_right <- p_right_list$plot
    ...
    if (!inherits(md_by, "try-error") && "median" %in% names(md_by)) {
...
invisible(
  purrr::walk(
    split(summary_main_models_v2, seq_len(nrow(summary_main_models_v2))),
    ~ save_main_plot(.x)
  )
)
save_adjusted_plot <- function(row) {
  # Build a safe filename but keep directory separators intact
  base_name <- paste0("adjplot_", row$outcome, "__", row$covariate, "_v2.tiff") |> 
    gsub("[^A-Za-z0-9_.-]+", "_", x = _)
  out <- file.path("output/figures/adjusted", base_name)
  # For survival outcomes, draw KM plots (overall and by stage) with colored median lines
  if (exists("km_labels") && length(km_labels) && as.character(row$outcome) %in% km_labels) {
    sp <- km_specs[[which(km_labels == as.character(row$outcome))[1]]]
    d <- df |>
      dplyr::select(time = dplyr::all_of(sp$time), event = dplyr::all_of(sp$event), stage = dplyr::all_of(col_stage)) |>
      dplyr::mutate(time = suppressWarnings(readr::parse_number(as.character(.data$time))),
                    event = ensure_binary_event(.data$event),
    ...
    p_left <- survminer::ggsurvplot(
      f_all, data = d, conf.int = TRUE, risk.table = FALSE,
      ggtheme = theme_minimal() + theme(plot.title = element_text(hjust = 0.5, face = "bold")),
      title = paste0("KM (All): ", sp$label, " | covariate=", as.character(row$covariate)),
      palette = pal_all
    )$plot
    md_all <- try(survminer::surv_median(f_all), silent = TRUE)
    if (!inherits(md_all, "try-error") && length(md_all$median) >= 1 && is.finite(md_all$median[1])) {
    ...
    p_right_list <- survminer::ggsurvplot(
      ...
      ggtheme = theme_minimal() + theme(plot.title = element_text(hjust = 0.5, face = "bold")),
      legend.title = "stage_early",
      title = paste0("KM by stage_early: ", sp$label, " | covariate=", as.character(row$covariate))
    )
    p_right <- p_right_list$plot
    str_order <- unique(as.character(p_right$data$strata))
    pal <- scales::hue_pal()(length(str_order))
    ...
    if (requireNamespace("cowplot", quietly = TRUE)) {
      p_comb <- cowplot::plot_grid(p_left, p_right, ncol = 2)
      ggplot2::ggsave(out, p_comb, width = 12, height = 8, dpi = 300, device = "tiff", compression = "lzw")
    } else if (requireNamespace("patchwork", quietly = TRUE)) {
      p_comb <- p_left + p_right + patchwork::plot_layout(ncol = 2)
      ggplot2::ggsave(out, p_comb, width = 12, height = 8, dpi = 300, device = "tiff", compression = "lzw")
    } else {
      grDevices::tiff(out, width = 2200, height = 1000, res = 300, compression = "lzw")
  ...
  p_right <- if (nrow(df_right) > 0) make_point_plot(df_right, xlab = NULL, title = paste0(row$outcome, " ~ ", row$covariate, " (by stage_early)")) else NULL
  if (requireNamespace("cowplot", quietly = TRUE)) {
    p_comb <- if (!is.null(p_right)) cowplot::plot_grid(p_left, p_right, ncol = 2) else p_left
    ggplot2::ggsave(out, p_comb, width = if (is.null(p_right)) 10 else 12, height = 8, dpi = 300, device = "tiff", compression = "lzw")
  } else if (requireNamespace("patchwork", quietly = TRUE) && !is.null(p_right)) {
    p_comb <- p_left + p_right + patchwork::plot_layout(ncol = 2)
    ggplot2::ggsave(out, p_comb, width = 12, height = 8, dpi = 300, device = "tiff", compression = "lzw")
  } else {
    grDevices::tiff(out, width = 2000, height = 1000, res = 300, compression = "lzw")
...
if (exists("summary_adjusted_models")) {
  invisible(
    purrr::walk(
      split(summary_adjusted_models, seq_len(nrow(summary_adjusted_models))),
      ~ save_adjusted_plot(.x)
    )
  )
}
